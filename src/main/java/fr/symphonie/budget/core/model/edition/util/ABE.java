...
public class ABE extends Tab2 {
	...
	public void setDefaultsFor2024(){
		double TotalDepAe=0,TotalDepCp=0,solde=0;
			
			double recetGl=getRecette().getR11()+getRecette().getR26()+getRecette().getR12()+getRecette().getR13()+getRecette().getR14()+getRecette().getR15();
			getRecetGlobal().setMontantDouble(recetGl);
			double recetFle=getRecette().getR22()+getRecette().getR27()+getRecette().getR24()+getRecette().getR25();
			getTotalRecetFleche().setMontantDouble(recetFle);
			getTotalRecette().setMontantDouble(recetGl+recetFle);
			for(int i=0; i<=4;i++){
				TotalDepAe+=getDepence().getMontantAE(i);
